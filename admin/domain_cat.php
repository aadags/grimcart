...
<li class="first"><a href="myaccount.php">My Password</a></li>
       ...
       <hr align="center" noshade="noshade" size="2" width="100%">
		...
		if(isset($_POST['maincat1']))
			{
			     $catname = $adfunc->test_input($_POST['categoryname']);
				 
				 $caturlname = $adfunc->test_input($_POST['urlname']);
				 $idcat = $adfunc->test_input($_POST['catid']);
				  if($catname == false)
                    {
                       echo "<p style='color:#F00'><strong>category name empty!</strong></p>";
                    }
					else if($caturlname == false)
                    {
                       echo "<p style='color:#F00'><strong>category url name empty!</strong></p>";
                    }
					else
					{
						$sql = mysqli_query($connect->connection, "UPDATE `categories` SET `cat_name`='$catname',`urlname`='$caturlname' WHERE `categoryID`=$idcat");
				...
				if(isset($_POST['maincat']))
        {
			...
			$getm = mysqli_query($connect->connection, "SELECT * FROM categories WHERE categoryID='$mcat'");
			while ($getmcat = mysqli_fetch_assoc($getm))
            {
			$mcatname = $getmcat['cat_name'];
			$mcaturl = $getmcat['urlname'];
			}
       ...
       <b style="color:#03F">hint--(urlname) the same with the category name..but with no spaces.e.g Videos/Audio---videosaudio</b><br/>
       ...
       <td>
       <input value="<?php echo $mcaturl; ?>" type='text' name='urlname' id='mcategoryname' maxlength="50" /><br/>    
       </td>
       </tr>
       <tr>
       <td>
       <input type="hidden" name="catid" value="<?php echo $mcat; ?>"  />
       <input type='submit' name="maincat1"  value='SAVE' />
       </td>
       <td></td>
       </tr>
       </table>
